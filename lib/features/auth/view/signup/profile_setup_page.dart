...
class ProfileSetupPage extends StatelessWidget {
                  ...
                  children: [
                    Obx(
                      () => Stack(
                        ...
                        children: [
                          GestureDetector(
                            onTap: authController.isUploadingPhoto
                                ? null
                                : authController.handleOnboardingPhotoUpload,
                            child: Container(
                              width: 128,
                              height: 128,
                              decoration: BoxDecoration(
                              ...
                              child: authController.isUploadingPhoto
                                  ? const CircularProgressIndicator()
                                  : _buildPhotoWidget(authController),
                            ),
                          ),
                        ],
                      ),
                    ...
                    Obx(
                      () => TextButton(
                        onPressed: authController.isUploadingPhoto
                            ? null
                            : authController.handleOnboardingPhotoUpload,
  ...
  Widget _buildPhotoWidget(AuthController authController) {
    // Priority: 1. Onboarding photo URL (just uploaded), 2. User profile photo, 3. Local path, 4. Initials
    if (authController.onboardingPhotoUrl != null &&
        authController.onboardingPhotoUrl!.isNotEmpty) {
      return ClipOval(
        child: _buildImageWidget(authController.onboardingPhotoUrl!),
      );
    }
    ...
    if (authController.onboardingPhotoLocalPath.isNotEmpty) {
      return ClipOval(
        child: _buildImageWidget(authController.onboardingPhotoLocalPath),
      );
    }
