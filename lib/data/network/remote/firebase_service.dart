...
class FirebaseService {
  ...
  /// Report an event with a reason. Stores in [event_reports] collection.
  Future<void> reportEvent(String eventId, String userId, String reason) async {
    try {
      await _firestore
          .collection(AppConstants.eventReportsCollection)
          .add({
        'event_id': eventId,
        'user_id': userId,
      ...
      throw Exception('Error reporting event: $e');
    }
  }

  /// Returns true if this user has already reported this event.
  Future<bool> hasUserReportedEvent(String eventId, String userId) async {
    try {
      final snapshot = await _firestore
          .collection(AppConstants.eventReportsCollection)
          .where('event_id', isEqualTo: eventId)
